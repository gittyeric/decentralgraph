...
type ErigonAPI interface {
	...
	GetBlockNode(ctx context.Context, minBlockNumber rpc.BlockNumber, maxBlockNumber rpc.BlockNumber) (interface{}, error)
...
func NewErigonAPI(base *BaseAPI, ethAPI *APIImpl, db kv.RoDB, eth rpchelper.ApiBackend) *ErigonImpl {
	return &ErigonImpl{
		BaseAPI:    base,
		eth:        ethAPI,
		db:         db,
